...
__device__ inline Frag pfrag(State *start, Ptrlist *out) {
  Frag n = {start, out};
  return n;
}

/* Create singleton list containing just outp. */
__device__ inline Ptrlist *plist1(State **outp) {
  Ptrlist *l;
...
__device__ inline void ppatch(Ptrlist *l, State *s) {
  Ptrlist *next;
...
__device__ inline Ptrlist *pappend(Ptrlist *l1, Ptrlist *l2) {
  Ptrlist *oldl1;
...
__device__ inline State *ppost2nfa(char *postfix) {
  char *p;
