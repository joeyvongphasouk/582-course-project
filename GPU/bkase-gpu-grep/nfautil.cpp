...
void copyStateToDevice(State **device_start, State *out, int pos) {
  ...
  if ((out != NULL)) { // && out->free != STATE_COPIED)) {
    ...
    if (out->dev == NULL) {
      ...
      // if out->dev is not null, then we have already traversed out->out and
      // out->out1 so stop
      done = 1;
    }
    ...
    if (pos == 0)
      cudaMemcpy(&((*device_start)->out), &device_out, sizeof(State *),
                 cudaMemcpyHostToDevice);
    else {
      cudaMemcpy(&((*device_start)->out1), &device_out, sizeof(State *),
                 cudaMemcpyHostToDevice);
    }
...
void copyStringsToDevice(char **lines, int numLines, char **device_line,
                         u32 **device_table) {

  // TODO: Is it more efficient to do this in two passes or to realloc a bunch
  // of times
  // TODO: Instead of making some tableOfLineStarts empty use a different index
  // for the avoided line[i]
  int size = 0;
  ...
  for (i = 0; i < numLines; i++) {
    // the size of this line is the runningTotal at the next index minus the
    // runningTotal at this one (subtract off the null byte)
  ...
  cudaMemcpy(*device_table, tableOfLineStarts, sizeof(u32) * (numLines + 1),
             cudaMemcpyHostToDevice);
...
void readFile (char *fileName, char ***lines, int *lineIndex) {
        ...
        int numRead = 0;
        while (!feof(fp) && (numRead = fread(line, sizeof(char), readSize, fp) >
0)) { readSize *= 2; size = size + readSize; head = (char *) realloc(head,
size);
...
void parseCmdLine(int argc, char **argv, int *visualize, int *postfix,
                  int *time, int *simplified, char **fileName,
                  char **regexFile) {
  if (argc < 3) {
    usage(argv[0]);
    exit(EXIT_SUCCESS);
  ...
  static struct option long_options[] = {
      {"help", no_argument, 0, '?'},       {"postfix", no_argument, 0, 'p'},
      {"simplified", no_argument, 0, 's'}, {"visualize", no_argument, 0, 'v'},
      {"file", required_argument, 0, 'f'}, {"regex", required_argument, 0, 'r'},
      {"time", no_argument, 0, 't'},       {0, 0, 0, 0}};

  *visualize = 0;
  *postfix = 0;
  *time = 0;
  *simplified = 0;
  while ((opt = getopt_long_only(argc, argv, "tvpsf:r:?", long_options,
                                 NULL)) != EOF) {
