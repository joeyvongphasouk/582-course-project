...
__global__ void parallelMatch(char *bigLine, u32 *tableOfLineStarts,
  ...
  if (threadIdx.x == 0) {
    pre2post(regexLines + regexTable[0]);
    pnstate = 0;
    states = s;
    st = ppost2nfa(buf);
  }
  ...
  __syncthreads();
  List d1, d2;
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < numLines; i += gridDim.x * blockDim.x) {
...
void pMatch(char *bigLine, u32 *tableOfLineStarts, int numLines, int numRegexs,
  ...
  cudaMemset(devResult, 0, numLines * sizeof(unsigned char));

  int blockSize = 256;  // Optimal for Ampere architecture
  int gridSize = (numLines + blockSize - 1) / blockSize;
  
  // Launch kernel with optimal configuration
  parallelMatch<<<gridSize, blockSize>>>(bigLine, tableOfLineStarts, numLines, 
                                        numRegexs, time, regexLines, regexTable, 
                                        devResult);
